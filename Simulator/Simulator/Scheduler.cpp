...
void Scheduler::treatJobInQueue(JobQueue jobQueue, Node node, int time) {
	...
	if (nodeType == 0) {
		node.useNodes(node.traditionalNodes, job.nbHours, time, job.nbNodes);
	} else if (nodeType == 1) {
		node.useNodes(node.acceleratedNodes, job.nbHours, time, job.nbNodes);
	} else if (nodeType == 2) {
		node.useNodes(node.specializedNodes, job.nbHours, time, job.nbNodes);
	}
	else {
